...
async function restore() {
    try {
        ...
        const USER = {
            ...
            projects: [
                {
                    ...
                    color: ["#7C3AED", "#22D3EE", "#4ADE80"],   // BorderGlow colors
                    glowColor: "258 89 73",                    // HSL for edge glow
            ...
            certs: [
                {
                    ...
                    src: '#',      // TODO: paste your certificate image URL or hosted PDF link here
                    isImage: true  // true = image preview, false = iframe (for PDF/web links)
                },
                {
                ...
                },
                {
                    ...
                    src: '#',
                    isImage: false // Coursera link → iframe
                },
            ],
            ...
            // Critical Admin fields
            admin_password: "2130",
            securityAnswer: "7"
        };
        
        // Preserve existing certificates if they exist to not lose the user's recent uploads
        const existingDoc = await UserConfig.findOne({ id: 'single_user' });
        if (existingDoc && existingDoc.data && existingDoc.data.certificates) {
            USER.certificates = existingDoc.data.certificates;
            console.log("Preserved existing certificates:", USER.certificates.length);
        } else if (existingDoc && existingDoc.data && existingDoc.data.certs) {
            USER.certificates = existingDoc.data.certs;
        } else {
        ...
        console.log('Restored USER data successfully');
        process.exit(0);
    } catch (e) {
        console.error(e);
        process.exit(1);
